...
void XmlConfig::printTree(std::ostream &out, XMLNode *node) {
  ...
  if (!strcmp(el->Name(), "obj")) {
    out << el->Name() << " type=" << el->Attribute("type") << " name "
        << el->Attribute("name") << std::endl;
...
std::deque<Config::ObjDev> XmlConfig::findObjDevs(std::string name,
                                                  PWR_AttrName attr) {
...
std::deque<std::string> XmlConfig::findAttrChildren(std::string name,
                                                    PWR_AttrName attr) {
  DBGX2(DBG_CONFIG, "%s %s\n", name.c_str(), attrNameToString(attr).c_str());
...
XMLNode *XmlConfig::findNodeWithAttr(XMLElement *elm, std::string nodeName,
                                     std::string attrName,
...
std::string XmlConfig::objTypeToString(PWR_ObjType type) {
  ...
  case PWR_OBJ_PLATFORM:
    return "Platform";
  case PWR_OBJ_CABINET:
    return "Cabinet";
  case PWR_OBJ_CHASSIS:
    return "Chassis";
  case PWR_OBJ_BOARD:
    return "Board";
  case PWR_OBJ_NODE:
