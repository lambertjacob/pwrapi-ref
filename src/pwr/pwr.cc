...
int PWR_CntxtInit(PWR_CntxtType type, PWR_Role role, const char *name,
                  PWR_Cntxt *ctx) {
...
int PWR_ObjAttrGetValue(PWR_Obj obj, PWR_AttrName type, void *ptr,
                        PWR_Time *ts) {
...
int PWR_ObjAttrGetValues(PWR_Obj obj, int num, PWR_AttrName attrs[],
  ...
  return OBJECT(obj)->attrGetValues(num, (PWR_AttrName *)attrs, values, ts,
                                    STATUS(status));
}
...
int PWR_ObjAttrGetSamples(PWR_Obj obj, PWR_AttrName name, PWR_Time *start,
...
int PWR_ObjAttrGetSamples_NB(PWR_Obj obj, PWR_AttrName name, PWR_Time *start,
                             double period, unsigned int *count, void *buf,
                             PWR_Request req) {
  return OBJECT(obj)->attrGetSamples(name, start, period, count, buf,
                                     REQUEST(req));
...
int PWR_GrpDuplicate(PWR_Grp, PWR_Grp *dup) { return PWR_RET_NOT_IMPLEMENTED; }
...
int PWR_GrpGetNumObjs(PWR_Grp group) { return GRP(group)->size(); }
...
int PWR_GrpAttrSetValues(PWR_Grp grp, int num, PWR_AttrName attr[], void *buf,
                         PWR_Status status) {
...
int PWR_GrpAttrGetValues(PWR_Grp grp, int num, PWR_AttrName attr[], void *buf,
                         PWR_Time ts[], PWR_Status status) {
  return GRP(grp)->attrGetValues(num, (PWR_AttrName *)attr, buf, ts,
                                 STATUS(status));
}
...
int PWR_StatusClear(PWR_Status status) { return STATUS(status)->clear(); }
...
int PWR_MetaValueAtIndex(PWR_Obj, PWR_AttrName, unsigned int index, void *val,
                         char *val_str) {
  return PWR_RET_NOT_IMPLEMENTED;
}
...
int PWR_StatStart(PWR_Stat stat) { return STAT(stat)->start(); }

int PWR_StatStop(PWR_Stat stat) { return STAT(stat)->stop(); }

int PWR_StatClear(PWR_Stat stat) { return STAT(stat)->stop(); }
...
int PWR_StatGetReduce(PWR_Stat, PWR_AttrStat, int *index, double *val,
                      PWR_TimePeriod *) {
  return PWR_RET_NOT_IMPLEMENTED;
}

int PWR_GetMajorVersion() { return PWR_MAJOR_VERSION; }

int PWR_GetMinorVersion() { return PWR_MINOR_VERSION; }
...
int PWR_ReqWait(PWR_Request req) { return static_cast<Request *>(req)->wait(); }
...
PWR_Request PWR_ReqCreateCallback(PWR_Cntxt ctx, PWR_Status status,
  ...
  return new DistRequest(static_cast<Cntxt *>(ctx), STATUS(status), callback,
                         data);
}
